...
static void PatchCEXKernel(void)
{
    ...
    *patch = 0x00000000; // NOP
    ...
    for (i = 0; i < 64; i++, patch++) {
        if ((patch[0] & ResetPattern_mask[0]) == ResetPattern[0] && (patch[1] & ResetPattern_mask[1]) == ResetPattern[1] && (patch[2] & ResetPattern_mask[2]) == ResetPattern[2] && (patch[3] & ResetPattern_mask[3]) == ResetPattern[3]) {
    ...
    for (i = 0; i < 32; i++, patch++) {
        if ((patch[0] & SMODE1ResetPattern_mask[0]) == SMODE1ResetPattern[0] && (patch[1] & SMODE1ResetPattern_mask[1]) == SMODE1ResetPattern[1] && (patch[2] & SMODE1ResetPattern_mask[2]) == SMODE1ResetPattern[2] && (patch[3] & SMODE1ResetPattern_mask[3]) == SMODE1ResetPattern[3]) {
